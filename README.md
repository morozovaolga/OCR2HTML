...
- Запустите конвейер:
  ...
  - С опциональными шагами:
    - добавить LanguageTool (облако) и пост‑очистку (склейка «В ы р е з к а», «обни мают», латиница→кириллица):
      - python pdf_to_epub.py --pdf path/to/file.pdf --outdir out --title "Мой документ" --lt-cloud --post-clean --epub-template sample.epub
    - **Рекомендуется:** `--lt-cloud --natasha-sync` для лучшего качества (79.61% точности) — см. [интерактивный дашборд с результатами тестирования](https://morozovaolga.github.io/ocr2epub/)
